  ...
  <div>
    ...
    <div id="list-body">
      ...
      <div>
        ...
        <table>
          <thead>
            <tr>
              <td>序号</td><td>客户名称</td><td>负责人</td><td>电话</td><td>手机</td><td>邮箱</td><td>地址</td><td>备注</td>
            </tr>
          </thead>
          <tbody>
    ...
    <div id="list-foot">
      <p>
        ...
        <button v-on:click="addCustomer">新增</button>
        <span>
...
<script>
import slaesApi from '../../../api/sales'
export default {
  ...
  mounted(){
    this.customerData()    //当页面加载时执行supplierAllData方法
  },
  methods: {
    customerData: function(){    //获取供应商数据
      slaesApi.customerData(this.queryData).then(
        response => {
    ...
    addCustomer: function(){
      this.$router.push({ path:'/sales/addCustomer' })
    }
  }
}
</script>